...
void setup() {
  ...
  // UART2 for HC-05 Bluetooth communication
  ...
  // Verify MPU6050 is responding on I2C bus
...
void loop() {
  ...
  // Sends live readings every loop cycle to the operator's phone
  // Useful for monitoring without opening the Wi-Fi dashboard
...
float calculateRisk(float vib, float dist, float temp) {
  ...
  // Distance sub-score — note: closer = higher score (inverted mapping)
  float distScore;
...
void updateBuzzer(unsigned long now) {
  if (alertLevel == 0) {
    // Safe — ensure buzzer is always off
    digitalWrite(BUZZER_PIN, LOW);
    buzzerState = false;

  } else if (alertLevel == 1) {
    // Warning — short beep pattern
...
void sendBluetoothAlert() {
  Serial2.println("==== CROWDGUARD ALERT ====");
...
// handleRoot()
// ────────────
// Serves the complete dashboard HTML page when someone opens the ESP32's IP
// The page auto-fetches /data every 400ms and updates all elements live
// No external libraries, no CDN — fully self-contained single page
void handleRoot() {
...
function drawGraph(h){
  ...
  // Background zones — red above 70, orange 40-70
...
void handleData() {
  ...
  for (int i = 0; i < HISTORY_SIZE; i++) {
    int idx = (historyIndex + i) % HISTORY_SIZE; // Read in chronological order
